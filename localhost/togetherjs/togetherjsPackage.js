...
define('ui',["require", "jquery", "util", "session", "templates", "templating", "linkify", "peers", "windowing", "tinycolor", "elementFinder", "visibilityApi"], function (require, $, util, session, templates, templating, linkify, peers, windowing, tinycolor, elementFinder, visibilityApi) {
  ...
  ui.prepareUI = function () {
    ...
    if(navigator.userAgent.toLowerCase().indexOf('chrome') == -1 || webkitNotifications.checkPermission() == 0)   //check if it is Chrome or if the user has already allowed notifications
      ui.container.find("#togetherjs-menu-web-notifications").hide();       //Hide the menu entry
  };
...
define('chat',["require", "jquery", "util", "session", "ui", "templates", "playback", "storage", "peers", "windowing"], function (require, $, util, session, ui, templates, playback, storage, peers, windowing) {
  ...
  function displayNotification(msg){
   if(navigator.userAgent.toLowerCase().indexOf('chrome') > -1)   //check if the webbrowser is Chrome
      chrome_displayNotification(msg); 
    else 
      w3c_displayNotification(msg);     
  }

  //W3C Notifications (Firefox, Safari)

  function w3c_checkPermission(){
  ...
  function w3c_displayNotification(msg){
      if(w3c_checkPermission())
      {
           notification= new Notification(msg.peer.name, { 'body': msg.text});    //When constructed it shows the notification
  ...
  //Webkit Notifications (Chrome)

  function chrome_checkPermission(){
    if (webkitNotifications.checkPermission()==0)     //0 == default
      return true;
    else
      return false;
  }

  function chrome_displayNotification(msg){    
    if(chrome_checkPermission()){
      notification = webkitNotifications.createNotification(null, msg.peer.name, msg.text);
      notification.show();                                                  //Webkit needs to invoke show function to display
